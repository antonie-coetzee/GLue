...
#define bufferInit(BUF, S, T,MEMALLOC) \
  BUF.size = S+1; \
  BUF.start = 0; \
  BUF.end = 0; \
  BUF.elems = (T*)MEMALLOC(BUF.size * sizeof(T))

#define bufferResize(BUF, S, T,FREE,MEMALLOC) \
  BUF.size = S+1; \
  BUF.start = 0; \
  BUF.end = 0; \
  FREE(BUF.elems); \
...
#define bufferClear(BUF) \
  BUF->start = 0; \
  BUF->end = 0; 

#define bufferDestroy(BUF,FREE) FREE(BUF->elems)
...
#define bufferCount(BUF, CNT) \
  if (!isBufferEmpty(BUF)) { \
    if(BUF->start < BUF->end){ \
		CNT = BUF->end - BUF->start; \
	} \
	else \
	{ \
		CNT = (BUF->size - BUF->start) + BUF->end; \
	} \
  } \
  else \
  {	\
